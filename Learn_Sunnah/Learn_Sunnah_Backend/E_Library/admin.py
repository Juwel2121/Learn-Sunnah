...
from .models import (Book, BookCategory, Rating, Review, User, UserProfile, Question, QuestionCategory, Rank, Comment, Reply,
                     PROFILE_ADMIN, PROFILE_MODERATOR, PROFILE_USER, STATUS_PUBLISHED, STATUS_PENDING, STATUS_REJECTED,
                     QUESTION_ANSWERED, QUESTION_REJECTED, QUESTION_UNANSWERED, QUESTION_TYPE_PUBLIC, QUESTION_TYPE_PRIVATE)
...
class QuestionProfileAdmin(admin.ModelAdmin):
    list_display = ['question', 'author', 'category', 'question_status', 'question_type']
    list_editable = ['category', 'question_status']
    list_per_page = 10
    ordering = ['question']

    def get_queryset(self, request: HttpRequest) -> QuerySet[Any]:
        # qs1 = Question.objects.filter(question_status=QUESTION_UNANSWERED).all().order_by('-created_at', 'question_type')
        # qs2 = Question.objects.filter(question_status=QUESTION_ANSWERED).all().order_by('-created_at', 'question_type')
        # qs3 = Question.objects.filter(question_status=QUESTION_REJECTED).all().order_by('-created_at', 'question_type')
        # qs = qs1.union(qs2)
